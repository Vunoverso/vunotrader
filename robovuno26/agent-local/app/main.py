...
def send_heartbeat(bridge: BridgeFilesystem, client: BackendClient) -> None:
    payload = {
        ...
        "details": {
            ...
            "pending_metadata": len(bridge.pending_metadata()),
        },
    }
...
def process_metadata(
    ...
    for metadata_path in bridge.pending_metadata():
        try:
            ...
            bridge.archive(metadata_path)
            continue

        detected_symbols = payload.get("available_symbols") or payload.get("market_watch_symbols") or []
        if not isinstance(detected_symbols, list):
            print(f"[SKIP] Metadata sem catalogo valido: {metadata_path.name}")
            bridge.archive(metadata_path)
            continue

        try:
            response = client.send_symbol_catalog(payload)
        except Exception as exc:
        ...
        print(
            "[METADATA] "
            f"{response.get('primary_symbol') or payload.get('chart_symbol') or '-'} "
            f"-> {len(detected_symbols)} simbolo(s) detectado(s)"
        )
