...
- %APPDATA%\MetaQuotes\Terminal\Common\Files\<bridge_da_instancia>\metadata
...
Depois que o EA ligar, ele passa a exportar automaticamente ao painel:

- ativo principal do gráfico atual
- timeframe do gráfico
- símbolos disponíveis no terminal/corretora
- símbolos atualmente monitorados pela instância
...
- timeframe operacional = o timeframe do grafico onde o EA foi anexado
- InpSnapshotCandles = quantidade de candles fechados enviados no snapshot do timeframe operacional
- InpHigherTimeframe = timeframe de confirmacao enviado ao backend
- InpHigherTimeframeCandles = quantidade de candles fechados enviados do timeframe de confirmacao
- InpAllowRealTrading = false enquanto estiver em homologação

Contrato atual do snapshot:

- os arrays `candles` e `htf_candles` saem em ordem cronologica
- o ultimo item do array e o candle fechado mais recente
- `close`, `ema_fast`, `ema_slow` e `rsi` usam o candle fechado mais recente, nunca o candle ainda em formacao
...
- catálogo de símbolos aparecendo no painel da instância
