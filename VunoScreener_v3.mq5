...
void OnTimer()
{
   ...
   for(int i=0; i<count; i++) 
   {
      ...
      if(currentBar != arrLastBar[i] && currentBar > 0) 
      {
         ...
         // Se modo efetivo era observer, descarta trade após treinar brain
...
{
   int s = SocketCreate();
   if(s==INVALID_HANDLE) return "";
   if(!SocketConnect(s, PythonHost, PythonPort, 3000)) { SocketClose(s); return ""; }
   uchar data[]; StringToCharArray(message, data, 0, StringLen(message));
   if(SocketSend(s, data, ArraySize(data)) < 0) { SocketClose(s); return ""; }
   string resp=""; uchar b[]; ArrayResize(b, 65536);
   uint tOut = GetTickCount() + 5000;
   while(GetTickCount() < tOut) {
      uint a = SocketIsReadable(s);
      if(a > 0) {
         int rec = SocketRead(s, b, a, 2000);
         if(rec > 0) { resp += CharArrayToString(b, 0, rec); break; }
      }
      Sleep(10);
   }
   SocketClose(s);
